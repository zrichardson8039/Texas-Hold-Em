...
/**
 * PokerTableTest represents a poker table that has a predetermined distribution
 * of cards that allows for testing of the methods in the PokerTable Class.
 * 
...
public class PokerTableTest {
    ...
    //variables to be toggled in the order the methods should be called
    private boolean canFlop; 
    private boolean canTurn;
    private boolean canRiver;
    
    //Set to true if the user constructs with a predetermined scenario to be tested
    ...
    /**
     * Constructor that accepts predetermined values of each card in the playing hand for testing purposes.
     * 
     * @param player1
     * @param player2
     * @param player3
     ...
     * @param river 
     */
    public PokerTableTest(Card[] player1, Card[] player2, Card[] player3, Card[] flop, Card turn, Card river)
    {
        players = new ArrayList<>();
        
        players.add(new PokerHand());
        players.get(0).addCard(player1[0]);
        players.get(0).addCard(player1[1]);
        
        players.add(new PokerHand());
        players.get(1).addCard(player2[0]);
        players.get(1).addCard(player2[1]);
        
        players.add(new PokerHand());
        players.get(2).addCard(player3[0]);
        players.get(2).addCard(player3[1]);
    ...
    {    
        ...
        int[] kickers = new int[winners.size()]; //Stores the next highest kicker of each PokerHand
        int i = 0;
        ...
        //Only retains the PokerHands with the highest kicker of the current index
        for(PokerHand currWinner : winners)
        {
